    ...
    def register_resolver(
        ...
        def resolver_wrapper(
            ...
            if args_as_strings and any(not isinstance(k, str) for k in key):
                warnings.warn(
                    f"Resolver '{name}' was registered with `args_as_strings=True` "
                    f"(current default value) and was given at least one "
                    f"non-string argument. For now we preserve the original string "
                    f"representation of such arguments, but this behavior is "
                    f"deprecated. Please register '{name}' with "
                    f"`args_as_strings=False`, ensuring that it does not assume "
                    f"string-only inputs anymore. This will become the default "
...
def _make_hashable(key: Tuple[Any, ...]) -> Tuple[Any, ...]:
    ...
    for idx, item in enumerate(key):
        ...
        if new_key is None:
            # Initialize `new_key` from the first elements of `key` that we skipped over.
            new_key = list(key[0:idx])
        new_key.append(hashable_item)
    assert new_key is not None  # otherwise the very first check should have succeeded
...
def _safe_cmp(x: Any, y: Any) -> int:
    ...
    try:
        ...
        if type_x == type_y:
            # cannot compare two different objects of the same type!?
