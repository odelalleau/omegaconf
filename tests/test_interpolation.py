...
TEST_CONFIG_DATA: List[Tuple[str, Any, Any]] = [
    ...
    ("truefalse", "${test:true_false}", "true_false"),
    ("unquoted_str_space", "${test:hello world}", "hello world"),
    ("unquoted_str_esc_space", r"${test:\ hello\ world\ }", " hello world "),
    ("unquoted_str_esc_comma", r"${test:hello\, world}", "hello, world"),
    ("unquoted_other_char", f"${{test:{chr(200)}}}", InterpolationSyntaxError),
    ("unquoted_emoji", f"${{test:{chr(129299)}}}", InterpolationSyntaxError),
    ("unquoted_dot", "${test:.}", "."),
    ("unquoted_esc", r"${test:\{}", InterpolationSyntaxError),
    ("quoted_str_single", "${test:'!@#$%^&*()[]:.,\"'}", '!@#$%^&*()[]:.,"',),
    ("quoted_str_double", '${test:"!@#$%^&*()[]:.,\'"}', "!@#$%^&*()[]:.,'",),
    ("quote_outer_ws_single", "${test: '  a \t'}", "  a \t"),
    ("int", "${test:123}", 123),
    ("int_pos", "${test:+123}", 123),
    ("int_neg", "${test:-123}", -123),
    ("int_underscore", "${test:1_000}", 1000),
    ("int_underscore_bad_1", "${test:1_000_}", "1_000_"),
    ("int_underscore_bad_2", "${test:1__000}", "1__000"),
    ("int_underscore_bad_3", "${test:_1000}", "_1000"),
    ("int_zero_start", "${test:007}", "007"),
    ("float", "${test:1.1}", 1.1),
    ...
    ("float_exp_bad_3", "${test:1e+03}", "1e+03"),
    ...
    ("dict_as_input_1", "${test:{a: 1.1, b: b}}", {"a": 1.1, "b": "b"}),
    ("dict_as_input_2", "${test:{'a': 1.1, b: b}}", InterpolationSyntaxError),
    ("dict_typo_colons", "${test:{'a': 1.1, b:: b}}", InterpolationSyntaxError,),
    ("dict_unhashable", "${test:{[0]: 1}}", InterpolationSyntaxError),
    ...
    # Unmatched braces.
    ("missing_brace", "${test:${prim_str}", InterpolationSyntaxError),
    ...
    ("str_no_other", "${test:hi_${prim_str_space}}", "hi_hello world"),
    (
        "str_quoted_double",
        '${test:"I say "${prim_str_space}}',
        InterpolationSyntaxError,
    ),
    (
        "str_quoted_single",
    ...
    ("str_legal_noquote", "${test:a/-\\+.$*, \\\\}", ["a/-\\+.$*", "\\"]),
    ("str_equal_noquote", "${test:a,=b}", InterpolationSyntaxError),
    ("str_quoted_equal", "${test:a,'=b'}", ["a", "=b"]),
    ("str_quoted_too_many_1", "${test:''a'}", InterpolationSyntaxError),
    ("str_quoted_too_many_2", "${test:'a''}", InterpolationSyntaxError),
    ("str_quoted_too_many_3", "${test:''a''}", InterpolationSyntaxError),
    # Unquoted strings (within interpolations).
    ("str_dollar", "${test:$}", "$"),
    ("str_dollar_inter", "${test:$$${prim_str}}", "$$hi"),
    ("str_colon", "${test::}", ":"),
    ("str_ws", "${test:a b\tc  \t\t  d}", "a b\tc  \t\t  d"),
    ("str_backslash_noesc", r"${test:ab\cd}", r"ab\cd"),
    ("str_esc_inter_1", r"${test:\${foo\}}", InterpolationSyntaxError),
    ("str_esc_inter_2", r"${test:\${}", InterpolationSyntaxError),
    ("str_esc_brace", r"${test:$\{foo\}}", InterpolationSyntaxError),
    ("str_esc_backslash", r"${test:\\}", "\\"),
    ("str_esc_ws", "${test:\\ \\,\\\t}", " ,\t"),
    ("str_esc_quotes", "${test:\\'\\\"}", InterpolationSyntaxError),
    ("str_esc_many", r"${test:\\,\,\{,\]\null}", InterpolationSyntaxError),
    ("str_esc_mixed", r"${test:\,\:\\\{foo\}\[\]}", InterpolationSyntaxError),
    ...
    ("dict_nan_as_key", "${test:{${test:nan}: 0}}", InterpolationTypeError),
    ("empties", "${test:[],{}}", [[], {}]),
    (
        "structured_mixed",
    ...
    (
        "structured_deep_2",
        '${test:{null: [0, 3.14, false], true: {"a": [0, 1, 2], "b": {}}}}',
        InterpolationSyntaxError,
    ),
    ...
    ("nested_resolver", "${${test}:a, b, c}", ["a", "b", "c"]),
    (
        "nested_resolver_combined",
        "${te${test_partial}:a, b, c}",
        InterpolationSyntaxError,
    ),
    # Whitespaces.
    ...
    ("ws_inter_res_inside_id", "${te st:foo}", InterpolationSyntaxError),
    ...
    ("str_other_left", "${test:.:${prim_str_space}}", ".:hello world"),
    ...
    ("interpolation_in_quoted_str", "'${'${test:prim_str}", InterpolationSyntaxError,),
    # Special IDs as keys.
    ("None", {"True": 1}, ...),
    ...
    ("str_otheronly_noquote", "${test:a, .:}", ["a", ".:"]),
