...
TEST_CONFIG_DATA: List[Tuple[str, Any, Any]] = [
    ...
    ("list_access_bad_negative", "${prim_list.-1}", InterpolationSyntaxError),
    ...
    ("null_like_key_quoted_1", "${'None'.'True'}", InterpolationSyntaxError),
    ("null_like_key_quoted_2", "${'None.True'}", InterpolationSyntaxError),
    ("dotpath_bad_type", "${prim_dict.${float}}", InterpolationTypeError),
    ...
    ("dict_as_input_quotes", "${test:{'a': 1.1, b: b}}", InterpolationSyntaxError),
    ("dict_typo_colons", "${test:{a: 1.1, b:: b}}", {"a": 1.1, "b": ": b"}),
    ("dict_list_as_key", "${test:{[0]: 1}}", InterpolationSyntaxError),
    ...
    ("str_quoted_concat_bad_1", '${test:"Hi "${prim_str}}', InterpolationSyntaxError),
    ("str_quoted_concat_bad_2", "${test:'Hi''there'}", InterpolationSyntaxError),
    ...
    ("str_legal", "${test:a/-\\+.$*, \\\\}", ["a/-\\+.$*", "\\"]),
    ("str_illegal_1", "${test:a,=b}", InterpolationSyntaxError),
    ("str_illegal_2", f"${{test:{chr(200)}}}", InterpolationSyntaxError),
    ("str_illegal_3", f"${{test:{chr(129299)}}}", InterpolationSyntaxError),
    ("str_dot", "${test:.}", "."),
    ("str_dollar", "${test:$}", "$"),
    ("str_colon", "${test::}", ":"),
    ...
    ("str_esc_illegal_1", r"${test:\{}", InterpolationSyntaxError),
    ("str_esc_illegal_2", r"${test:\${foo\}}", InterpolationSyntaxError),
    ("str_esc_illegal_3", "${test:\\'\\\"}", InterpolationSyntaxError),
    # Whitespaces.
    ...
    ("dict_int_key", "${test:{0: 0}}", InterpolationSyntaxError),
    ("dict_float_key", "${test:{1.1: 0}}", InterpolationSyntaxError),
    ("dict_nan_key_1", "${test:{nan: 0}}", InterpolationSyntaxError),
    ("dict_nan_key_2", "${test:{${test:nan}: 0}}", InterpolationTypeError),
    ("dict_null_key", "${test:{null: 0}}", InterpolationSyntaxError),
    (
        "dict_bool_key",
    ...
    ("int_chain", "${int}", 123),
    ("list_chain_bad_1", "${${prim_list}.0}", InterpolationTypeError),
    ("dict_chain_bad_1", "${${prim_dict}.a}", InterpolationTypeError),
    ("prim_list_copy", "${prim_list}", OmegaConf.create([-1, "a", 1.1])),
    ("prim_dict_copy", "${prim_dict}", OmegaConf.create({"a": 0, "b": 1})),
    ("list_chain_bad_2", "${prim_list_copy.0}", ConfigKeyError),
    ("dict_chain_bad_2", "${prim_dict_copy.a}", ConfigKeyError),
    ...
    # Unmatched braces.
    ("missing_brace_1", "${test:${prim_str}", InterpolationSyntaxError),
    ("missing_brace_2", "${${test:prim_str}", InterpolationSyntaxError),
